...
LUA_API int lua_gctime(lua_State *L, int idx)
{
    global_State *g = G(L);
    return g->gcCostTimeArray[idx % 100];
}

LUA_API int lua_gccosttime(lua_State *L, int reset)
{
    global_State *g = G(L);
    int ret = g->gcCostTime;
    if (reset == 1)
    {
        g->gcCostTime = 0;
    }
...
LUA_API int lua_gcCount(lua_State *L, int reset)
{
    global_State *g = G(L);
    int ret = g->gcCount;
    if (reset == 1)
...
LUA_API int lua_stacksize(lua_State *L) { return L->stacksize; }
