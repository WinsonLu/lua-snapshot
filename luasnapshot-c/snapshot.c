...
static void traverse_object(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent, const char* link);
static void traverse_table(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent, const char* link);
static void traverse_function(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent, const char* link);
static void traverse_userdata(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent, const char* link);
static void traverse_thread(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent, const char* link);
...
static void mark_function_env(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent)
{
    lua_getfenv(L, -1);
    if (lua_istable(L, -1)) {
...
static const char* lua_type_to_string[] = {
    NULL, "boolean", "lightuserdata", "number", "string",
    "table", "function", "userdata", "thread", "numtags"
};
*/

static struct lua_gc_node* gen_node(lua_State* L, lua_State* dL,
    struct lua_gc_node* parent,
    const char* link)
{
    ...
    size_t size)
{
    ...
    case LUA_TNIL:
        ...
        break;
    default:
        snprintf(buffer, size, "[%s:%p]", lua_typename(L, type),
    ...
    struct lua_gc_node* parent, const char* link)
{
    ...
    // 如果是_G表且link不是_G，则跳过该节点
    lua_getglobal(L, "_G");
    if (lua_rawequal(L, -1, -2) && strcmp(link, "_G") != 0) {
        lua_pop(L, 2);
        return;
    }
    lua_pop(L, 1);
...
static int lua_gc_node_gc(lua_State* L)
{
...
static int snapshot_printjson(lua_State* L, bool is_formatted)
{
    if (lua_gettop(L) != 1) {
    ...
    if (node == NULL)
        return 0;
    char* jsonstr = is_formatted ? lua_gc_node_to_jsonstrfmt(node)
                                 : lua_gc_node_to_jsonstr(node);
    printf("%s\n", jsonstr);
    cJSON_free(jsonstr);
    return 0;
}

static int snapshot_print_jsonfmt(lua_State* L)
{
    return snapshot_printjson(L, true);
}

static int snapshot_print_json_nofmt(lua_State* L)
{
    return snapshot_printjson(L, false);
}

static int snapshot_tojsonfile(lua_State* L, bool is_formatted)
{
    if (lua_gettop(L) != 2) {
...
static int snapshot_tojsonfile_nofmt(lua_State* L)
{
    return snapshot_tojsonfile(L, false);
}

static int snapshot_tojsonfile_fmt(lua_State* L)
{
    return snapshot_tojsonfile(L, true);
}

static int snapshot_tofile(lua_State* L)
{
    if (lua_gettop(L) != 2) {
        luaL_error(L, "Number of arguments should be 2.");
        return 0;
...
static int snapshot_print(lua_State* L)
{
    if (lua_gettop(L) != 1) {
        luaL_error(L, "Number of arguments should be 1.");
        return 0;
...
static int snapshot_free(lua_State* L)
{
    if (lua_gettop(L) != 1) {
        luaL_error(L, "Number of arguments should be 1.");
        return 0;
...
static int snapshot_copy(lua_State* L)
{
    if (lua_gettop(L) != 1) {
        luaL_error(L, "Number of arguments should be 1.");
        return 0;
...
static int snapshot_diff(lua_State* L, bool isAdded)
{
    if (lua_gettop(L) != 2) {
        luaL_error(L, "Number of arguments should be 2.");
        return 0;
...
static int snapshot_increased(lua_State* L) { return snapshot_diff(L, true); }

static int snapshot_decreased(lua_State* L) { return snapshot_diff(L, false); }

static struct luaL_Reg snapshot_lib[] = {
    { "snapshot",
        snapshot }, // 保存当前时刻的某一table或registry的快照，并返回一个snapshot(userdata)对象
    { "print", snapshot_print }, // 转换为字符串，并打印
    { "print_jsonfmt",
        snapshot_print_jsonfmt }, // 转换为格式化过的json字符串，并打印
    { "print_json",
        snapshot_print_json_nofmt }, // 转换为未格式化过的json字符串, 并打印
    { "to_file", snapshot_tofile }, // 转换为字符串，并输出到指定文件
    { "to_jsonfile",
        snapshot_tojsonfile_nofmt }, // 转换为未格式化过的json字符串，并输出到指定文件
    { "to_jsonfilefmt",
        snapshot_tojsonfile_fmt }, // 转换为格式化过的json字符串，并输出到指定文件
    { "free", snapshot_free }, // 手动释放snapshot所占用的内存
    { "copy", snapshot_copy }, // 复制snapshot
    { "incr", snapshot_increased }, // 求出snapshot1 到 snapshot2
    // 的增量，返回类型仍为snapshot
    { "decr", snapshot_decreased }, // 求出snapshot1 到 snapshot2
    // 的减量，返回类型仍为snapshot
    { NULL, NULL }
};
